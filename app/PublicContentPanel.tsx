...
export default function PublicContentPanel({ data: initial, initialTab }: { data: PublicContentData; initialTab: "downloads" | "news" }) {
  ...
  return <div className={cn("foundation-page", moduleSurface)}>
    ...
    {tab === "downloads" ? <div className="config-layout">
      <form className="config-card config-form" onSubmit={addDownload}><div className="card-title"><h2>Publish a download</h2><p>Large files upload in secure parts and become available when the upload is complete.</p></div><label>Public title<input name="title" required maxLength={120} /></label><label>Description<textarea name="description" maxLength={500} rows={4} /></label><label>Campus<select name="campusId"><option value="">All campuses</option>{data.campuses.map(c => <option value={c.id} key={c.id}>{c.name}</option>)}</select></label><label>File<input name="file" type="file" required /></label>{uploadProgress !== null && <div className="upload-progress" aria-live="polite"><div><span>Uploading file</span><strong>{uploadProgress}%</strong></div><progress max="100" value={uploadProgress} /></div>}<button className="primary" disabled={busy || !data.canManage}>{busy ? `Uploading${uploadProgress !== null ? ` ${uploadProgress}%` : ""}…` : "Publish download"}</button></form>
