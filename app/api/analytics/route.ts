...
export async function GET(request:Request){
 ...
 const [students,staff,attendance,fees,results,operations,classes]=await Promise.all([
  bind("SELECT count(*) value FROM students WHERE organization_id=?1 AND enrollment_status='active' AND (?2 IS NULL OR home_campus_id=?2)").first<{value:number}>(),
  ...
  bind("SELECT round(avg(m.percentage),1) average,count(*) results FROM assessment_marks m JOIN assessments a ON a.id=m.assessment_id WHERE m.organization_id=?1 AND (?2 IS NULL OR a.campus_id=?2) AND a.status IN ('approved','published')").first<{average:number|null;results:number}>(),
 ...
 return Response.json({...payload,canExport:auth.permissions.has("analytics.export")},{headers:{"cache-control":"private, no-store"}});
  ...
  return Response.json({error:"Analytics is temporarily unavailable. Please try again."},{status:503,headers:{"cache-control":"private, no-store"}});
 }
}
