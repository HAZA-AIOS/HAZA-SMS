...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const object=await env.BUCKET.get(row.r2_key);if(!object)return new Response(null,{status:404});return new Response(object.body as unknown as ReadableStream<Uint8Array>,{headers:{'content-type':row.content_type,'cache-control':'private, no-store','x-content-type-options':'nosniff'}});
}
