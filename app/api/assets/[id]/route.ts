...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const asset=await env.DB.prepare("SELECT r2_key,original_name,content_type,campus_id FROM storage_assets WHERE id=?1 AND organization_id=?2").bind(id,auth.organizationId).first<{r2_key:string;original_name:string;content_type:string;campus_id:string|null}>();
  if(!asset)return Response.json({error:"File not found."},{status:404});
  if(asset.campus_id){const denied=await requireCampusAccess(auth,asset.campus_id,"asset.download");if(denied)return denied;}
